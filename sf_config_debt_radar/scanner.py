...
def scan_metadata_xml(
    xml_text: str, config: dict[str, Any] | None = None
) -> dict[str, Any]:
    config = config or {}
...
def metadata_findings(
    ...
    for name, info in entities.items():
        if (
            name in classified["core_ec"]
            or name in classified["custom_mdf"]
            or name in classified["foundation"]
        ):
            ...
            if info["custom_field_count"] >= custom_field_limit and not name.startswith(
                "PerGlobalInfo"
            ):
                findings.append(
                    {
            ...
            if (
                name in classified["custom_mdf"]
                and not info["has_effective_start_date"]
            ):
                findings.append(
                    {
                        "severity": "MEDIUM",
...
def run_count_checks(
    client: SFClient, metadata_result: dict[str, Any], config: dict[str, Any]
) -> list[dict[str, Any]]:
    findings = []
    ...
    for entity in ("EmpJob", "Position", "EmpEmployment"):
        ...
        fields = [
            f
            for f in ec_entities.get(entity, {}).get("fields", [])
            if f.get("nullable")
        ][:max_fields]
        for field in fields:
            name = field["name"]
            ...
            if total and null_count is not None:
                ...
                if null_rate >= float(
                    config.get("thresholds", {}).get("blank_rate_high", 95)
                ) and (field.get("is_custom") or "custom" in name.lower()):
                    findings.append(
                        {
                            "severity": "HIGH",
                            "area": "Custom Fields",
    ...
    event_reason_entities = [
        e for e in ("EventReason", "FOEventReason") if e in entities
    ]
